...
// Ensure initialStatusFilter defaults to "All" if not provided
function AllTickets({ onSelect, token, refresh, initialStatusFilter = "All" }) {
  const [tickets, setTickets] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [unitFilter, setUnitFilter] = useState("All");

  // This state MUST be initialized with the prop and updated when the prop changes
  ...
  useEffect(() => {
    // CRITICAL: Update the internal statusFilter state when the prop changes.
    // This makes sure the component reacts to route changes properly.
    if (statusFilter !== initialStatusFilter) {
      setStatusFilter(initialStatusFilter);
    }

    // Reset other filters if you want them to clear when switching categories
    // setUnitFilter("All");
    // setTypeFilter("All");
  ...
  }, [refresh, initialStatusFilter]); // Only refresh and initialStatusFilter should trigger re-fetch/sync

  // handleDelete, handleSelectAll, handleSelectTicket, handleDeleteSelected functions remain the same
  ...
  // Filtering logic uses the statusFilter state
    ...
    <Container className="mt-4">
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <h2 className="mb-4 text-primary">
          ...
          {/* Dynamically update the heading based on the current filter */}
          {statusFilter === "All"
            ? "All Tickets"
        ...
        <Row className="mb-3">
          ...
          <Col xs={12} md={4} lg={3}>
            <Form.Select
              value={statusFilter} // This dropdown controls the *current* filter
