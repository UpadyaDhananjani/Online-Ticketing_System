...
const adminHome = () => { // Changed component name to PascalCase for convention
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            setError("");
            try {
                // Ensure these API calls are correctly configured and accessible
                // If your backend requires authentication, make sure these endpoints are protected
                // and that the client sends cookies (axios.defaults.withCredentials = true is set in AppContext)
        ...
        // Set up interval to refetch data every 30 seconds if user is logged in
    ...
    }, [userData]); // Re-run effect if userData changes (e.g., user logs in/out)
    ...
    // Render message if user is not logged in (and not loading)
    if (!userData) {
        return (
            <Container className="py-5">
    ...
    // Styles (kept for consistency, though some might be redundant with Tailwind/Bootstrap)
    const boxStyle = {
