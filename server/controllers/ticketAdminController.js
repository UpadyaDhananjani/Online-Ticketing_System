...
export const getAllTickets = async (req, res) => {
  try {
    // --- CRUCIAL FIX: Uncomment and set to populate 'name' ---
    const tickets = await Ticket.find({})
      .populate('user', 'name') // Populate the 'user' field and select only the 'name'
      .sort({ createdAt: -1 }); // Sort by creation date descending

    // Remove userEmail mapping, as frontend will directly access ticket.user.name
    res.json(tickets); // Return the tickets directly with populated user objects
  } catch (err) {
