...
    // Optional: You might want to add role-based authorization here
    // For example, checking if the logged-in user is an admin
    // if (req.user.role !== 'admin') {
    //     return res.status(403).json({ message: 'Access denied.' });
    // }

    // Validate the incoming priority against the enum values in the model
    const validPriorities = ['Low Priority', 'Medium Priority', 'High Priority', 'Critical Priority', 'Normal Priority'];
    if (!validPriorities.includes(priority)) {
      return res.status(400).json({ message: 'Invalid priority value.' });
    }

    // Find the ticket by ID and update the priority
    const updatedTicket = await Ticket.findByIdAndUpdate(
      id,
      { priority, updatedAt: Date.now() }, // Update priority and the updatedAt timestamp
